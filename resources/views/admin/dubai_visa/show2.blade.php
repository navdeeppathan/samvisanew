...
@section('title', 'UAE / Dubai Visa — ' . $application->surname . ', ' . $application->first_name)
...
<style>
  ...
  /* ── HERO ── */
  .av-hero {
    background: linear-gradient(135deg, var(--uae-green-dark), var(--uae-green), var(--uae-red));
  ...
  .av-hero-flag { position: relative; z-index: 1; flex-shrink: 0; }
  ...
  .av-card-icon { width: 40px; height: 40px; border-radius: 10px; background: var(--uae-green); display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
  ...
  .doc-card:hover { border-color: var(--uae-green); box-shadow: 0 6px 24px rgba(0,98,51,.15); }
  ...
  .doc-thumb.gb-thumb  { background: linear-gradient(135deg,#eff6ff,#dbeafe); }
  ...
  /* ── INFO BOXES ── */
  .info-box { padding: 14px 18px; border-radius: 10px; font-size: 12px; line-height: 1.7; margin-top: 16px; }
...
<div class="av-wrap">
  ...
  <div class="av-topbar">
    <div class="av-breadcrumb">
      ...
      <span>/</span>
      <a href="{{ route('admin.visa.show.all') }}">UAE / Dubai Applications</a>
      <span>/</span>
     ...
     <div class="av-actions">
      ...
      <form 
        method="POST" 
        action="{{ route('admin.dubai-visa.approve', $application->id) }}" 
      ...
      <form
       method="POST" 
       action="{{ route('admin.dubai-visa.reject', $application->id) }}" 
       style="display:inline"
       >
        @csrf @method('PATCH')
      ...
      <button class="btn-action btn-edit" onclick="window.location.href='{{ route('admin.dubai.visa.show', $application->id) }}'">
  ...
  <div class="av-hero">
    <div class="av-hero-flag">
      <img src="{{ asset('dubaiflag.svg') }}" alt="UAE Flag"/>
    </div>
    <div class="av-hero-info">
      <p class="av-hero-sub">UAE / Dubai Visit Visa Application #{{ $application->id }}</p>
  ...
  <div class="av-grid">
    ...
    <div>
      ...
      <div class="av-card">
        <div class="av-card-header">
          ...
          <div>
            ...
            <p class="av-card-sub">Parents' names as required by UAE embassy</p>
          </div>
        </div>
        <div class="av-card-body">
          <div class="fv-grid fv-2">
          ...
          <div class="info-box blue" style="margin-top:12px;">
            ℹ️ Both parents' names are provided as required by the UAE embassy for identity verification.
          </div>
        </div>
      </div>
      ...
      <div class="av-card">
        <div class="av-card-header">
          ...
          <div>
            ...
            <p class="av-card-sub">Trip dates, destination & accommodation</p>
          </div>
        </div>
        <div class="av-card-body">
          <div class="fv-grid fv-2">
      ...
      <div class="av-card">
        ...
        {{-- <div class="av-card-body">
          @php
            $docs = [
              ['key' => 'passport_scan',   'label' => 'Passport Scan',        'icon' => '🛂',  'required' => true,  'thumb_class' => ''],
              ['key' => 'passport_photo',  'label' => 'Passport Size Photo',  'icon' => '🤳',  'required' => true,  'thumb_class' => ''],
              ['key' => 'uk_visa_copy',    'label' => 'UK Visa / BRP Copy',   'icon' => '🇬🇧', 'required' => true,  'thumb_class' => 'gb-thumb'],
              ['key' => 'bank_statements', 'label' => 'Bank Statements',      'icon' => '🏦',  'required' => false, 'thumb_class' => ''],
              ['key' => 'payslips',        'label' => 'Payslips',             'icon' => '💷',  'required' => false, 'thumb_class' => ''],
            ];
          @endphp
          <div class="doc-grid">
            @foreach($docs as $doc)
              @if($application->{$doc['key']})
                ...
                <a href="{{ asset('storage/'.$path) }}" target="_blank" class="doc-card">
                  <div class="doc-thumb {{ in_array($ext,['pdf']) ? 'pdf-thumb' : $doc['thumb_class'] }}">
        ...
        <div class="av-card-body">
            ...
            @if($application->{$doc['key']})
                ...
                <div class="doc-thumb 
                    {{ $ext === 'pdf' ? 'pdf-thumb' : $doc['thumb_class'] }}">

                    {{-- IMAGE --}}
                    @if(in_array($ext, ['jpg','jpeg','png','webp']))
                    <img src="{{ asset($file) }}" alt="{{ $doc['label'] }}" style="object-fit:fill;"/>

                    {{-- PDF --}}
                    @elseif($ext === 'pdf')
                    <div style="padding:10px;">
                        <span style="font-size:28px;">📄</span>
                        <p style="font-size:12px;margin-top:5px;">PDF Preview</p>
                    </div>
      ...
      <div class="av-card">
        <div class="av-card-header">
          ...
          <div>
            ...
            <p class="av-card-sub">eVisa sharecode for UAE embassy verification</p>
          </div>
        </div>
        <div class="av-card-body">
          ...
          @if($application->uk_sharecode)
            <div class="info-box green">
              ✅ Sharecode provided. Verify at <strong>gov.uk/view-prove-immigration-status</strong> using the applicant's date of birth and the code above.
            </div>
          @else
            <div class="info-box amber" style="margin-top:12px;">
              ⚠️ No sharecode submitted. Applicant may be a British citizen, or may need to be contacted to provide their eVisa sharecode.
            </div>
          @endif
        </div>
      </div>
      ...
      <div class="av-card">
        ...
        <div class="av-card-body">
          <div style="background:var(--gray-bg);border-radius:10px;padding:18px 20px;font-size:13px;line-height:1.7;color:var(--gray-dark);white-space:pre-line;border-left:3px solid var(--uae-green);">{{ $application->message }}</div>
        </div>
      </div>
      @endif
      ...
      <div class="av-card">
        <div class="av-card-header">
          ...
          <div>
            ...
            <p class="av-card-sub">Applicant's final confirmation at submission</p>
          </div>
        </div>
        <div class="av-card-body">
          <div style="display:flex;align-items:center;gap:14px;padding:16px 20px;background:#f0fdf4;border:1.5px solid #86efac;border-radius:12px;">
            ...
            <span style="font-size:13px;color:#14532d;font-weight:500;line-height:1.6;">Applicant confirmed that all information and documents provided are genuine, true, and accurate. They acknowledged that false information may result in visa refusal or ban.</span>
          </div>
        </div>
      </div>
    ...
    <div class="av-sidebar">
      ...
      {{-- SUMMARY --}}
      <div class="side-card" style="background:linear-gradient(135deg,var(--uae-green-dark),var(--uae-green));border-color:transparent;">
      ...
      {{-- @if($application->passport_expiry)
        ...
        <div class="side-card" style="border-color:#fca5a5;background:#fff5f5;">
          ...
          <p style="font-size:12px;color:#7f1d1d;line-height:1.6;">
            ...
            @else
              Passport expires in <strong>{{ $expDays }} days</strong>. UAE requires 6 months validity — please advise the applicant.
            @endif
          </p>
        </div>
        @endif
      ...
      {{-- <div class="side-card">
        ...
        <div style="display:flex;flex-direction:column;gap:8px;">
          ...
          @if($application->uk_sharecode)
          <a href="https://www.gov.uk/view-prove-immigration-status" target="_blank" style="display:flex;align-items:center;gap:10px;padding:11px 14px;border-radius:10px;background:#f0fdf4;border:1.5px solid #86efac;font-size:12px;font-weight:600;color:var(--green);text-decoration:none;">
